...
def generate_test_sample(filepath="test_flange_01.png"):
    ...
    # 3. Chipped edge (Fixed coordinates to overlap the metal boundary)
    chip_pts = np.array([[270, 205], [290, 225], [310, 200], [290, 175]], np.int32)
...
def run_inspection(image_path):
    ...
    if contours:
        ...
        #  Extract the actual solid silhouette of the object (fills internal holes)
        actual_mask = np.zeros_like(gray)
        cv2.drawContours(actual_mask, [main_contour], -1, 255, -1)

        #  Reconstruct a "perfect" ideal flange silhouette based on the measured center
        ideal_mask = np.zeros_like(gray)
        cx, cy = x + w // 2, y + h // 2
        # Draw the ideal rectangular base (height 120, extending 60 up/down from center)
        cv2.rectangle(ideal_mask, (x, cy - 60), (x + w, cy + 60), 255, -1)
        # Draw the ideal center circle (standard radius 160)
        cv2.circle(ideal_mask, (cx, cy), 160, 255, -1)

        # Template Subtraction: What exists in the ideal but is missing in the actual = edge chip
        # Note: cv2.subtract ensures values truncate to 0 instead of wrapping around to negatives
        diff_mask = cv2.subtract(ideal_mask, actual_mask)

        #  Morphological OPEN: Eliminates 1-2 pixel thin line noise from anti-aliasing/misalignment
        diff_mask = cv2.morphologyEx(diff_mask, cv2.MORPH_OPEN, kernel)

        #  Find the chip contours in the difference mask
        chip_contours, _ = cv2.findContours(diff_mask, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
        for cc in chip_contours:
            if cv2.contourArea(cc) > 50: # Filter out micro-errors; area > 50 is a true chip
                if "EDGE_CHIPPED" not in status_flags:
                    status_flags.append("EDGE_CHIPPED")
                cx_def, cy_def, cw_def, ch_def = cv2.boundingRect(cc)
                # Use a magenta box to specifically highlight the chipped edge
                cv2.rectangle(output_img, (cx_def, cy_def), (cx_def+cw_def, cy_def+ch_def), (255, 0, 255), 2)
                cv2.putText(output_img, "CHIPPED", (cx_def, cy_def-5), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 0, 255), 1)
